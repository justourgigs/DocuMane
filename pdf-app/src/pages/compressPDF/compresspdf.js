...
const CompressPDF = () => {
  ...
  const handleCompress = () => {
    ...
    axios
      ...
      .then((response) => {
        router.push({
          pathname: "/compressPDF/download",
          query: { url: encodeURIComponent(response.data) },
        });
      })
      .catch((error) => {
        toast.error("File Compression Unsuccessful!");
        console.log(error);
      });
  };
  ...
  return (
    <>
      <ToastContainer />
      <Head>
        <title>DocuMane - Compress PDF</title>
      </Head>
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl">
          <div className="flex justify-between items-center mb-6">
            <span className="font-bold text-lg px-12">Compress PDF</span>
            <button
